...
export const UsuarioService = {
  porId: async (idUsuario) => {
    try {
      ...
      if (error.response) {
        throw new Error(
          error.response.data.mensagem || "Erro ao buscar usuário"
        );
      }
  ...
  buscarPorParametro: async (param, grupo) => {
    const res = await api
      .get(`api/usuario/listar/param/${grupo}/`, {
        params: {
          param,
          retorno: "dependencia",
        },
  ...
  listarPorGrupo: async (perfil, param, pagina) => {
    try {
      ...
      if (error.response) {
        throw new Error(
          error.response.data.mensagem || "Erro ao buscar usuários"
        );
      }
