  ...
  const filtrarPEDs = () => {
    const PEDsFiltradas = listaPED_ProEJA.filter((ped) => {
      ...
      return (
        ...
        (!status ||
          removeAcentos(ped.status).toLowerCase() ===
            removeAcentos(status).toLowerCase()) &&
        (!situacao ||
          removeAcentos(ped.situacao).toLowerCase() ===
            removeAcentos(situacao).toLowerCase())
      );
    });
