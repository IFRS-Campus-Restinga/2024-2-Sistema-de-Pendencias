...
def carregar_grupos_e_permissoes(apps, schema_editor):
    Group = apps.get_model('auth', 'Group')
    Permission = apps.get_model('auth', 'Permission')

    # Caminho dos arquivos .json
    caminho_grupos = os.path.join(BASE_DIR, 'grupos.json')
    caminho_permissoes = os.path.join(BASE_DIR, 'permissoes.json')

    # Carregar grupos
    with open(caminho_grupos, 'r') as f:
        grupos_data = json.load(f)

    # Carregar permissões
    with open(caminho_permissoes, 'r') as f:
        permissoes_data = json.load(f)

    # Dicionário para mapear permissões por ID
    permissoes_dict = {p['pk']: p for p in permissoes_data}

    # Criar os grupos e associar as permissões
    for grupo_data in grupos_data:
        grupo_nome = grupo_data['fields']['name']
        grupo, criado = Group.objects.get_or_create(name=grupo_nome)

        if criado:
            # Adicionar permissões ao grupo
            permissoes_ids = grupo_data['fields']['permissions']
            for perm_id in permissoes_ids:
                perm_data = permissoes_dict.get(perm_id)
                if perm_data:
                    permissao = Permission.objects.get(codename=perm_data['fields']['codename'])
    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('perfil', models.CharField(default='usuario', max_length=50)),
        ...
        migrations.AddField(
            model_name='curso',
            name='turma',
            field=models.ManyToManyField(to='dependencias_app.turma'),
        ),
        # Cria os grupos com as permissões
        migrations.RunPython(carregar_grupos_e_permissoes),
    ]
