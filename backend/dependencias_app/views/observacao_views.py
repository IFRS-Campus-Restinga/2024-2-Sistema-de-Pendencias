...
def adicionar_observacao(request):
    try:
        ...
        serializer = Observacao_Serializer(data=data)
...
def listar_observacoes(request):
    try:
        ...
        serializer = Observacao_Serializer(observacoes, many=True)
...
def editar_observacao(request, id):
    try:
        observacao = Observacao.objects.get(pk=id)
        observacao_serializer = Observacao_Serializer(observacao, data=request.data)
