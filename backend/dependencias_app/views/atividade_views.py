...
def buscar_atividade(request, modalidade, atividadeId):
    try:
        ...
        atividade = get_object_or_404(model, pk=atividadeId)

        serializer = serializer_class(atividade, context={'request': request})
        
        return Response(serializer.data, status=status.HTTP_200_OK)
    except Http404:
        return Response({'mensagem': 'Nenhuma atividade encontrada para este ID'}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        return Response({'mensagem': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def buscar_por_avaliacao(request, modalidade, avaliacaoId):
    try:
        ...
        return Response({'mensagem': 'Avaliação inexistente'}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        return Response({'mensagem': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

@api_view(['GET'])
@permission_classes([Professor | GestaoEscolar | Coordenador | Aluno])
# traz a lista de atividades vinculadas a uma progressão através do model Avaliação
def listar_atividades(request, pedId, modalidade):
    try:        
        ...
        atividades = model.objects.filter(ped=pedId).order_by('data_criacao')
        serializer = serializer_class(atividades, many=True, context={'request': request})
...
def vincular_atividades(request, pedId, modalidade):
    try:
        ...
        return Response({'mensagem': 'Nenhuma progressão encontrada para este ID'})
